...
$(document).ready(function() {
    ...
    $(document).on('change', 'input#single_dpp', function(e) {
        ...
        var add_p = __read_number($('input#add_p'));
        var mark_up = __read_number($('input#profit_percent'));
        var single_dpp = __read_number($('input#single_dpp'));

        if(tax_rate == 0)
        {
            var purchase_inc_tax = parseFloat(purchase_inc_tax);
        }
        else
        {
           //var percent = parseFloat(purchase_inc_tax) / parseFloat(tax_rate);
            var rate = (tax_rate / 100);

            var purchase_inc_tax = (parseFloat(single_dpp) * parseFloat(rate)) + parseFloat(single_dpp);
        }
        __write_number($('input#single_dpp'), single_dpp);
        __write_number($('input#single_dpp_inc_tax'), purchase_inc_tax);

        var m_percent = parseFloat(mark_up) / 100;

        var purchase_exc = (parseFloat(single_dpp) + parseFloat(add_p)) * parseFloat(m_percent);
        var purchase_inc = (parseFloat(purchase_inc_tax) + parseFloat(add_p)) * parseFloat(m_percent);

        var selling_price_exc_tax = (parseFloat(single_dpp) + parseFloat(add_p)) + parseFloat(purchase_exc);

        var selling_price_inc_tax = (parseFloat(purchase_inc_tax) + parseFloat(add_p)) + parseFloat(purchase_inc);

        __write_number($('input#single_dsp'), selling_price_exc_tax);
    ...
    $(document).on('change', 'select#tax', function() {
        if ($('select#type').val() == 'single') {
            ...
            if(tax_rate == 0)
            {
                var purchase_exc_tax = parseFloat(purchase_inc_tax);
            }
            else
            {
               //var percent = parseFloat(purchase_inc_tax) / parseFloat(tax_rate);
                var rate = (tax_rate / 100) + 1;

                var purchase_exc_tax = parseFloat(purchase_inc_tax) / parseFloat(rate);
            }
            ...
            var purchase_exc = (parseFloat(purchase_exc_tax) + parseFloat(add_p)) * parseFloat(m_percent);
            var purchase_inc = (parseFloat(purchase_inc_tax) + parseFloat(add_p)) * parseFloat(m_percent);

            var selling_price_exc_tax = (parseFloat(purchase_exc_tax) + parseFloat(add_p)) + parseFloat(purchase_exc);
    ...
    $(document).on('change', 'input#single_dpp_inc_tax', function(e) {
        ...
        //alert(purchase_inc_tax + add_p + purchase_inc)
        
    });

    //if add purchase change
    $(document).on('change', 'input#add_p', function(e) {
        var tax_rate = $('select#tax').find(':selected').data('rate');
        tax_rate = tax_rate == undefined ? 0 : tax_rate;
    ...
    $(document).on('change', 'input#single_dsp', function(e) {
        ...
        var purchase_exc = parseFloat(purchase_exc_tax) + parseFloat(add_p);

        //sp / p_inc - 1 * 100
        var selling_price_exc_tax = ((parseFloat(purchase_exc_tax) / parseFloat(purchase_exc)) - 1) * 100;

        __write_number($('input#profit_percent'), selling_price_exc_tax);

        //alert(selling_price_exc_tax)
    });

    //inc tax default selling price
    $(document).on('change', 'input#single_dsp_inc_tax', function(e) {
        ...
        var purchase_inc_tax = __read_number($('input#single_dpp_inc_tax'));
        var add_p = __read_number($('input#add_p'));
        var selling_price = __read_number($('input#single_dsp_inc_tax'));

        var purchase_inc = parseFloat(purchase_inc_tax) + parseFloat(add_p);

        //sp / p_inc - 1 * 100
        var selling_price_inc_tax = ((parseFloat(selling_price) / parseFloat(purchase_inc)) - 1) * 100;

        __write_number($('input#profit_percent'), selling_price_inc_tax);

        //alert(selling_price_inc_tax)
