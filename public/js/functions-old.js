...
function __add_percent(amount,additional_price, percentage = 0) {
    var amount = parseFloat(amount);
    var additional_price = parseFloat(additional_price);
    var percentage = isNaN(percentage) ? 0 : parseFloat(percentage);

    return (amount + additional_price) + (percentage / 100) * (amount + additional_price);
}
