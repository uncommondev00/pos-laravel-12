...
<section class="content no-print">
	<div class="row">
		<div class="@if(!empty($pos_settings['hide_product_suggestion']) && !empty($pos_settings['hide_recent_trans'])) col-md-10 col-md-offset-1 @else col-md-7 @endif col-sm-12">
			...
			<div class="col-sm-6">
				<div class="form-group" style="margin-bottom: 0px;">
					<div class="input-group">
						...
						<select name="select_location_id" id="select_location_id" class="form-control input-sm mousetrap" required autofocus>
			...
			<form action="{{ route('pos.store') }}" method="POST" id="add_pos_sell_form">
				@csrf
				...
				<input type="hidden" id="prev_points" name="prev_points" value="0">
				<input type="hidden" id="points_value" name="points_value" value="0">
				<input type="hidden" id="points_amount" name="points_amount" value="{{ $points->points_amount }}">
				<input type="hidden" id="points" name="points" value="{{ $points->points }}">
				<input type="hidden" id="points_tobe_added" name="points_tobe_added" value="0">
				<input type="hidden" id="points_status" name="points_status" value="0">
				<input type="hidden" id="points_is_suspend" value="1">
				...
				<div class="box-body">
						...
						<div class="col-md-4 col-sm-6">
							<div class="form-group">
								<div class="input-group">
									...
									@php
									reset($price_groups);
									$first_price_group_key = key($price_groups);
									@endphp
									<input type="hidden" id="hidden_price_group" name="hidden_price_group"
										value="{{ $first_price_group_key }}">
					...
					<!-- Customer Selection and Product Search -->
					<div class="row">
						<div class="@if(!empty($commission_agent)) col-sm-4 @else col-sm-6 @endif">
							<div class="form-group" style="width: 100% !important">
								<div class="input-group" id="form_default">
									...
									<!-- MAC Address -->
									<input type="hidden" name="mac_address" value="{{ $permit_Add_2 ?? ' ' }}">
									...
									<span class="input-group-btn">
										<button type="button" class="btn btn-default bg-white btn-flat add_new_customer mousetrap"
											id="customer_add" data-name=""
						...
						<!-- Payment Terms -->
						<input type="hidden" id="pay_term_number" name="pay_term_number" value="{{ $walk_in_customer['pay_term_number'] }}">
						<input type="hidden" id="pay_term_type" name="pay_term_type" value="{{ $walk_in_customer['pay_term_type'] }}">
							...
							<div class="form-group">
								<div class="input-group">
									...
									<input type="text" name="search_product" id="search_product"
										...
										{{ is_null($default_location) ? 'disabled' : 'autofocus' }}>
									<span class="input-group-btn">
										<button type="button" class="btn btn-default bg-white btn-flat pos_add_quick_product"
											...
											@if(!auth()->user()->can('product.create')) disabled @endif>
											<i class="fa fa-plus-circle text-primary fa-lg"></i>
										</button>
									</span>
								</div>
...
<script type="text/javascript">
	function opened() {
		...
		var routeUrl2 = "{{route('pos.create')}}"
	...
	$(document).ready(function() {
		...
		$('div#payment_rows_div').on('change', '.payment_types_dropdown', function() {
			...
			for (var index in payment_type_value) {
				...
				$('div#payment_rows_div #tr_row').find('option[value="' + payment_type_value[index] + '"]:not(:selected)').css({
					"color": "#C8C8C8"
				});
